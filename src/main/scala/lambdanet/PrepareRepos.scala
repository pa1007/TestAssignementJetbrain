...
import funcdiff.{SimpleMath, Statistics}
...
object PrepareRepos {
  ...
  def parseAndFilterDataSet(parsingFromFile: Path): Unit = {
  ...
  def parseRepos(
    ...
    val libDefs = if (loadFromFile) {
      ...
      val defs = parseLibDefs(parsingFromFile)
      SimpleMath.saveObjectToFile(libDefsFile.toIO)(defs)
      println(s"library definitions saved to $libDefsFile")
      defs
    }

    def fromDir(dir: Path, parsingFromFile: Path = pwd / RelPath("scripts/ts/parsingFromFile.js")) =
      (ls ! dir)
        .filter(f => f.isDir)
    ...
    libDefs -> dataSetDirs.map {
      b => fromDir(b, parsingFromFile)
    }
  }
  ...
  def parseLibDefs(parsingFromFile: Path): LibDefs = {
    ...
    val (baseCtx, libAllocator, defaultMapping, defaultModule) =
      QLangTranslation.parseDefaultModule(parsingFromFile)
  ...
  def parseProject(
                    ...
                    parsingFromFile: Path = pwd / RelPath("scripts/ts/parsingFromFile.js")
                  ): ParsedProject =
    SimpleMath.withErrorMessage(s"In project: $projectRoot") {
      ...
      Statistics.setStat("Nodes size", s"${graph.nodes.size}")
  ...
  def parseAndSerializeDataSet(parsingFromFile: Path): Unit = {
    ...
    val stats = repoStatistics(trainSet ++ devSet ++ testSet, parsingFromFile)
  ...
  def repoStatistics(results: Seq[ParsedProject], parsingFromFile: Path): RepoStats = {
    ...
    val data = rows.toMap
    RepoStats(average, data, parsingFromFile)
  }
  ...
  case class ParsedProject(
    ...
    def mergeEqualities: ParsedProject = {
      ...
      val qModules1 = qModules.map {
        _.mapNodes(merger)
      }
      val irModules1 = irModules.map {
        _.mapNodes(merger)
      }
  ...
  case class ParsedRepos(
    ...
    def serializeIntoDir(
      ...
      val allF = (libDefsF +: toSave.toVector.map {
        case (d, name) =>
          Future {
            SM.saveObjectToFile((dir / name).toIO)(d)
          }
      }).sequence_
