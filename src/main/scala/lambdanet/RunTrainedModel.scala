...
import funcdiff.{ParamCollection, Statistics}
...
object RunTrainedModel {
  ...
  def searchForFunctions(parsingFromFile: Path) = {
    ...
    val repos = DataSet.loadRepos(toyMode = false, parsingFromFile = parsingFromFile)
    val selected = for {
  ...
  def main(args: Array[String]): Unit = {
    ...
    runTrainedModel(paramPath, sourcePath, outputPath, pwd / RelPath("scripts/ts/parsingFromFile.js"))
  }
  ...
  def runTrainedModel(
    ...
    Statistics.setStat("Ram used by project", s"$l")
    ...
    val tes = Runtime.getRuntime.totalMemory() - Runtime.getRuntime.freeMemory()

    Statistics.setStat("Ram used by model", s"${(tes)}")
    ...
    val (_, fwd, pred) = model
      ...
      .get
    val tes2 = Runtime.getRuntime.totalMemory() - Runtime.getRuntime.freeMemory()
    Statistics.setStat("Ram used by dataset", s"${tes2}")
    ...
    Statistics.setStat("Lib Accuracy", s"${toAccuracy(fwd.libCorrect)}")
    println("projectAccuracy: " + toAccuracy(fwd.projCorrect))
    Statistics.setStat("Project Accuracy", s"${toAccuracy(fwd.projCorrect)}")
